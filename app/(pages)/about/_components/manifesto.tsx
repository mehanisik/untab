	...
	useGSAP(
		() => {
			...
			mm.add("(prefers-reduced-motion: no-preference)", () => {
				const eyebrowEl = root.querySelector<HTMLElement>(".about-eyebrow");
			...
			<div className="container px-6 md:px-12 lg:px-24">
				<h1 className="font-medium leading-[0.95] tracking-[-0.035em] text-foreground text-[clamp(2.25rem,5vw,4rem)] max-w-[14ch]">
					{headingLines.map((line) => (
						<span key={line} className="about-line block">
							{line}
						</span>
					))}
				</h1>
					...
					<div className="md:col-span-9 lg:col-span-8 space-y-6 max-w-2xl">
						{intro.map((paragraph, index) => (
							<p
								...
								className={
									index === 0
										? "about-body text-base md:text-lg leading-[1.65] text-foreground"
										: "about-body text-base md:text-lg leading-[1.65] text-muted-foreground"
								}
							>
								{paragraph}
							</p>
