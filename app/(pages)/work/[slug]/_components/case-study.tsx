...
import type { CaseStudy as CaseStudyData } from "../../_data";
...
export function CaseStudy({ study }: CaseStudyProps) {
	...
	return (
		<section
			ref={rootRef}
			className={`w-full pt-24 sm:pt-28 md:pt-32 lg:pt-36 pb-20 sm:pb-24 md:pb-32 ${PAGE_PADDING}`}
		...
		>
			<div className="grid grid-cols-1 gap-x-10 gap-y-10 sm:gap-y-12 lg:grid-cols-[1.35fr_1fr] lg:gap-x-14 xl:gap-x-20">
				<div className="min-w-0 order-2 lg:order-1">
					<ul ref={mediaRef} className="flex flex-col gap-3 md:gap-4">
						{study.media.map((item, index) => (
							<li
								key={`${item.kind}-${index}-${"src" in item ? item.src : ""}`}
								className="cs-media"
							>
								{item.kind === "image" ? (
				...
				>
					<h1 className="cs-headline text-[clamp(2.5rem,7vw,5rem)] font-medium leading-[0.92] tracking-[-0.035em] text-foreground">
						{study.title}
					</h1>
